...
### 切片
* 切片是从列表、元组、字符串等序列中提取子序列的操作，格式为`序列名[起始索引:终止索引:步长]`，将目标从序列中提取出来便于进行操作，通过调整步长可实现正向或逆向切片

### 文件读取与写入
#### 读取与使用
* `with open("文件名") as 变量名`用于打开文件`close("文件名")`用于关闭文件**若直接给出文件名，则会在当前执行的文件夹所在的目录中查找文件**，也可将地址赋给一个变量，用于节省空间例如：`file_path='/D:\Desktop\typora存档\typora.txt'
* `变量=文件变量名.read()`可将文件中的数据读取至变量中存放，read到末尾时会返回一个空字符串，打印出来时会多显示一行空行
* 若需要逐行读取，则可使用for循环，此时无需用.read()函数来读取，for循环中所设置的变量会读取，这将会以每次一行的方式检查文件，若读取后又无需变成多行，可使用字符串的相加让其变成一行，**每次检查后都会有一个空字符串**，可使用rstrip函数删除
* 每次读取文件的返回值是以字符串形式存储在变量中的，可使用if语句判断文件中是否存在某个指定的字符串
* 可以提取文本，`word=文件名.split()`可用于将文件中的内容将每个单词拆开并创建一个单词列表word，可以使用len()函数查看单词个数
* 使用多个文件时，可将全部文件名放入一个列表中，然后使用循环语句对每个文件执行操作
#### 写入
* 在打开时补充一个,'w'，表示以写入格式打开文件，使用`文件名.write(写入内容)`即可往文件中写入信息，
* **open函数可提供两个实参，第一个实参为打开的文件，第二个实参为打开格式，读取模式('r')，写入模式('w')，附加模式('a')，读取与写入模式('r+')，无第二个实参默认为读取模式('r')**
* 若需要写入多行，记得加入换行符
